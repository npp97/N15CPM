...
for (i in 2:length(flst)){
    ...
    if (length(which(is.na(c(diag(cor_s4_nh4_obs),diag(cor_s4_no3_obs),diag(cor_s4_nh4no3_obs)))))>1) next;
...
get_n2oc15<-function(time0,n2o,n2o_15){
  a2<-spline(x=time0,y=n2o,xout=seq(min(time0),max(time0)));
  a2$y=cumsum(a2$y);
  cn2o<-spline(x=a2$x,y=a2$y,xout=time0)$y;
  a3<-spline(x=time0,y=n2o*n2o_15/100,xout=seq(min(time0),max(time0)));
  a3$y<-cumsum(a3$y);
  cn2o_15<-spline(x=a3$x,y=a3$y,xout=time0)$y;
