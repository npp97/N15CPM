...
findBST<-function(flst){
  ...
  bestf0th.nh4no3<-quantile(bestf0.nh4no3,c(0.95));
  ...
  for (i in 2:length(flst)){
    ...
    if (length(which(is.na(s4.nh4[,'NH4'])))>0) next;
    if (length(which(is.na(c(diag(cor_s4_nh4_obs),diag(cor_s4_no3_obs),diag(cor_s4_nh4no3_obs)))))>0) next;
    if ((MCMC.NH4$bestfunp>bestf0th.nh4) | (MCMC.NO3$bestfunp > bestf0th.no3) | (MCMC.NH4NO3$bestfunp > bestf0th.nh4no3)) next;
...
parDist<-function(parMat){
  ...
  names(bs)<-names(qbs)<-names(parMat);
  l.p<-ncol(parMat)
  nmdist<-c('norm','lnorm','gamma','weibull')
  ...
  for (i in 1:l.p){
    ...
    l.fdr<-list(normf,lnormf,gammaf,weibullf);
    ca<-0;
    for (k in 1:4){if(class(l.fdr[[k]]) %in% 'try-error') ca<-c(ca,k);};ca<-ca[ca>0];
...
Z13acost <- function (pars,y0,times,obs) {
...
Z13acostL <- function (pars,y0,times,obs) {
  out <- ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='zhang2013a',nout=4);
  cost_1 <- modCost(model=out, obs=obs, weight='std',scaleVar =F)$minlogp
  return(cost_1)
}
Z13acostV <- function (pars,y0,times,obs) {
  out <- ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='zhang2013a',nout=4);
  cost_1 <- modCost(model=out, obs=obs, weight='std',scaleVar =F)$model
  return(cost_1)
}

Z13acost_3l <- function (pars,y0_nh4,y0_no3,y0_n4n3,times,obs_nh4,obs_no3,obs_n4n3) {
  out_nh4 <- as.data.frame(ode(y=y0_nh4,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='zhang2013a',nout=4));
  cost_nh4 <- modCost(model=out_nh4, obs=obs_nh4, weight='std',scaleVar =F)
  out_no3 <- as.data.frame(ode(y=y0_no3,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='zhang2013a',nout=4));
  cost_no3 <- modCost(model=out_no3, obs=obs_no3, weight='std',scaleVar =F,cost=cost_nh4);
  out_n4n3 <- as.data.frame(ode(y=y0_n4n3,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='zhang2013a',nout=4));
  cost_n4n3 <- modCost(model=out_n4n3, obs=obs_n4n3,weight='std',scaleVar =F,cost=cost_no3);
...
Z13acost_3ll <- function (pars,y0_nh4,y0_no3,y0_n4n3,times,obs_nh4,obs_no3,obs_n4n3) {
  out_nh4 <- as.data.frame(ode(y=y0_nh4,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='zhang2013a',nout=4));
  cost_nh4 <- modCost(model=out_nh4, obs=obs_nh4, weight='std',scaleVar =F)
  out_no3 <- as.data.frame(ode(y=y0_no3,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='zhang2013a',nout=4));
  cost_no3 <- modCost(model=out_no3, obs=obs_no3, weight='std',scaleVar =F,cost=cost_nh4);
...
Z13ecost <- function (pars,y0,times,obs) {
  out <- ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='zhang2013e',nout=4);
  cost_1 <- modCost(model=out, obs=obs, weight='std',scaleVar =F)
  return(cost_1)
}

Z13ecostL <- function (pars,y0,times,obs) {
  out <- ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='zhang2013e',nout=4);
  cost_1 <- modCost(model=out, obs=obs, weight='std',scaleVar =F)$minlogp
  return(cost_1)
}
Z13ecostV <- function (pars,y0,times,obs) {
  out <- ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='zhang2013e',nout=4);
  cost_1 <- modCost(model=out, obs=obs, weight='std',scaleVar =F)$model
  return(cost_1)
}
Z13e<-function(y0,pars,times){
  out <- ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='zhang2013e',nout=4);
  return(out);
}
...
Z13amcost_3l <- function (pars,y0_nh4,y0_no3,y0_n4n3,times,obs_nh4,obs_no3,obs_n4n3) {
  out_nh4 <- as.data.frame(ode(y=y0_nh4,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='zhang2013am',nout=4));
  cost_nh4 <- modCost(model=out_nh4, obs=obs_nh4, weight='std',scaleVar =F)
  out_no3 <- as.data.frame(ode(y=y0_no3,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='zhang2013am',nout=4));
  cost_no3 <- modCost(model=out_no3, obs=obs_no3, weight='std',scaleVar =F,cost=cost_nh4);
  out_n4n3 <- as.data.frame(ode(y=y0_n4n3,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='zhang2013am',nout=4));
  cost_n4n3 <- modCost(model=out_n4n3, obs=obs_n4n3,weight='std',scaleVar =F,cost=cost_no3);
#  cost_1<-(cost_nh4$model+cost_no3$model+cost_n4n3$model);
  cost_1<-cost_n4n3
  return(cost_1)
...
#
#
M07<-function(y0,pars,times){
  out <- ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='muller2003',nout=4);
  return(out);
}

M07cost <- function (pars,y0,times,obs) {
  out1 <- as.data.frame(ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='muller2003',nout=4));
  cost_1 <- modCost(model=out, obs=obs, weight='std',scaleVar =F)
  return(cost_1)
}


M07costL <- function (pars,y0,times,obs) {
  out <- ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='muller2003',nout=4);
  cost_1 <- modCost(model=out, obs=obs, weight='std',scaleVar =F)$minlogp
  return(cost_1)
}
M07costV <- function (pars,y0,times,obs) {
  out <- ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='muller2003',nout=4);
  cost_1 <- modCost(model=out, obs=obs, weight='std',scaleVar =F)$model
  return(cost_1)
}
...
M07cost_3l <- function (pars,y0_nh4,y0_no3,y0_n4n3,times,obs_nh4,obs_no3,obs_n4n3) {
  out_nh4 <- as.data.frame(ode(y=y0_nh4,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='muller2003',nout=4));
  cost_nh4 <- modCost(model=out_nh4, obs=obs_nh4, weight='std',scaleVar =F)
  out_no3 <- as.data.frame(ode(y=y0_no3,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='muller2003',nout=4));
  cost_no3 <- modCost(model=out_no3, obs=obs_no3, weight='std',scaleVar =F,cost=cost_nh4);
  out_n4n3 <- as.data.frame(ode(y=y0_n4n3,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='muller2003',nout=4));
  cost_n4n3 <- modCost(model=out_n4n3, obs=obs_n4n3,weight='std',scaleVar =F,cost=cost_no3);
#  cost_1<-(cost_nh4$model+cost_no3$model+cost_n4n3$model);
  cost_1<-cost_n4n3
  return(cost_1)
...
M07cost_3ll <- function (pars,y0_nh4,y0_no3,y0_n4n3,times,obs_nh4,obs_no3,obs_n4n3) {
  out_nh4 <- as.data.frame(ode(y=y0_nh4,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='muller2003',nout=4));
  cost_nh4 <- modCost(model=out_nh4, obs=obs_nh4, weight='std',scaleVar =F)
  out_no3 <- as.data.frame(ode(y=y0_no3,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='muller2003',nout=4));
  cost_no3 <- modCost(model=out_no3, obs=obs_no3, weight='std',scaleVar =F,cost=cost_nh4);
  ...
  cost_n4n3 <- modCost(model=out_n4n3, obs=obs_n4n3,weight='std',scaleVar =F,cost=cost_no3);
#  cost_1<-(cost_nh4$model+cost_no3$model+cost_n4n3$model);
  cost_1<-cost_n4n3$minlogp
  return(cost_1)
}

M03m<-function(y0,pars,times){
  out <- ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='muller2003m',nout=4);
  return(out);
}

M03mcost <- function (pars,y0,times,obs) {
  out1 <- as.data.frame(ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='muller2003m',nout=4));
  cost_1 <- modCost(model=out, obs=obs, weight='std',scaleVar =F)
  return(cost_1)
}


M03mcostL <- function (pars,y0,times,obs) {
  out <- ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='muller2003m',nout=4);
  cost_1 <- modCost(model=out, obs=obs, weight='std',scaleVar =F)$minlogp
  return(cost_1)
}
M03mcostV <- function (pars,y0,times,obs) {
  out <- ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='muller2003m',nout=4);
  cost_1 <- modCost(model=out, obs=obs, weight='std',scaleVar =F)$model
  return(cost_1)
}

M03mcost_3l <- function (pars,y0_nh4,y0_no3,y0_n4n3,times,obs_nh4,obs_no3,obs_n4n3) {
  out_nh4 <- as.data.frame(ode(y=y0_nh4,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='muller2003m',nout=4));
  cost_nh4 <- modCost(model=out_nh4, obs=obs_nh4, weight='std',scaleVar =T)
  out_no3 <- as.data.frame(ode(y=y0_no3,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='muller2003m',nout=4));
  cost_no3 <- modCost(model=out_no3, obs=obs_no3, weight='std',scaleVar =T,cost=cost_nh4);
  out_n4n3 <- as.data.frame(ode(y=y0_n4n3,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='muller2003m',nout=4));
  cost_n4n3 <- modCost(model=out_n4n3, obs=obs_n4n3,weight='std',scaleVar =T,cost=cost_no3);
#  cost_1<-(cost_nh4$model+cost_no3$model+cost_n4n3$model);
  cost_1<-cost_n4n3
  return(cost_1)
}

M03mcost_3la <- function (pars,y0_nh4,y0_no3,y0_n4n3,times,obs_nh4,obs_no3,obs_n4n3) {
  out_nh4 <- as.data.frame(ode(y=y0_nh4,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='muller2003m',nout=4));
  cost_nh4 <- modCost(model=out_nh4, obs=obs_nh4, weight='std',scaleVar =T)
  out_no3 <- as.data.frame(ode(y=y0_no3,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='muller2003m',nout=4));
  cost_no3 <- modCost(model=out_no3, obs=obs_no3, weight='std',scaleVar =T,cost=cost_nh4);
...
mdMCMC_3ml<-function(ff=M07cost_3l,pp1,lower0,upper0,obs_nh4,obs_no3,obs_n4n3,y0_nh4,y0_no3,y0_n4n3,times,var0=0.5,wvar0=0.1,niter1=35000,niter2=50000,updatecov=100,burninlength=10000)
{
  MCMC1 <- modMCMC(f=ff, p=pp1, niter=niter1,y0_nh4=y0_nh4,y0_no3=y0_no3,y0_n4n3=y0_n4n3,times=times,obs_nh4=obs_nh4,obs_no3=obs_no3,obs_n4n3=obs_n4n3,ntrydr = 3,var0=var0,
                   wvar0=0,updatecov=updatecov,lower=lower0, upper=upper0,burninlength=burninlength);
  if(MCMC1$naccepted<=10){
    
    MCMC1 <- modMCMC(f=ff, p=pp1, niter=niter2,y0_nh4=y0_nh4,y0_no3=y0_no3,y0_n4n3=y0_n4n3,times=times,obs_nh4=obs_nh4,obs_no3=obs_no3,obs_n4n3=obs_n4n3,ntrydr = 3,var0=var0,
                     wvar0=wvar0,updatecov=updatecov,lower=lower0, upper=upper0,burninlength=burninlength);
  }
  Cov0<-cov(MCMC1$pars)*2.4^2/(length(pp));
  # lower=(summary(MCMC1)[5,])*0.5;
  pp=unlist(summary(MCMC1)[6,]);
  MCMC <-modMCMC(f=ff, p=pp, niter=niter2,y0_nh4=y0_nh4,y0_no3=y0_no3,y0_n4n3=y0_n4n3,times=times,obs_nh4=obs_nh4,obs_no3=obs_no3,obs_n4n3=obs_n4n3,ntrydr = 3,jump=Cov0,var0=var0,
                 wvar0=wvar0,updatecov=updatecov,lower=lower0, upper=upper0,burninlength=burninlength);

  if(MCMC$naccepted<=10){
    MCMC <-modMCMC(f=ff, p=pp, niter=niter2*2,y0_nh4=y0_nh4,y0_no3=y0_no3,y0_n4n3=y0_n4n3,times=times,obs_nh4=obs_nh4,obs_no3=obs_no3,obs_n4n3=obs_n4n3,ntrydr = 3,var0=var0,
                   wvar0=wvar0,updatecov=updatecov,lower=lower0, upper=upper0,burninlength=burninlength);
   }
  return(MCMC);
}
...
M07f<-function(y0,pars,times){
  out <- ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='muller2003f',nout=4);
  return(out);
}

M07fcost <- function (pars,y0,times,obs) {
  out1 <- as.data.frame(ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='muller2003f',nout=4));
  cost_1 <- modCost(model=out1, obs=obs, weight='std',scaleVar =F)
  return(cost_1)
}
...
M07cost_s <- function (pars,y0,times,obs) {
  out<-ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='muller2003',nout=4);
  cost_1 <- modCost(model=out, obs=obs, weight='mean',scaleVar =F);
  return(cost_1)
}

M07_45<-function(y0,pars,times){
  out <- ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='muller2003_45',nout=4);
  return(out);
}

M07_45cost <- function (pars,y0,times,obs) {
  out <- as.data.frame(ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='muller2003_45',nout=4));
  cost_1 <- modCost(model=out, obs=obs, weight='std',scaleVar =F)
  return(cost_1)
}

ZHU16<-function(pars,y0,times){
  out <- ode(y=y0,parms=pars,times=times,func='derivs',method='ode45',initfunc='initmod',dllname='zhu2016',nout=4);
  return(out); 
}

ZHU16_cost<-function(pars,y0,times,obs){
  out <- ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='zhu2016',nout=4);
  cost_1 <- modCost(model=out, obs=obs, weight='std',scaleVar =F);
  return(cost_1)
}
ZHU16_costV<-function(pars,y0,times,obs){
  out <- ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='zhu2016',nout=4);
  cost_1 <- modCost(model=out, obs=obs, weight='std',scaleVar =F)$model
  return(cost_1)
}

ZHU16_costL<-function(pars,y0,times,obs){
  out <- ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='zhu2016',nout=4);
  cost_1 <- modCost(model=out, obs=obs, weight='std',scaleVar =F)$minlogp
  return(cost_1)
}
...
#model 
M07a<-function(y0,pars,times){
  out <- ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='muller2003a',nout=4);
  return(out);
}
M07acost <- function (pars,y0,times,obs) {
  out <- ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='muller2003a',nout=4);
  cost_1 <- modCost(model=out, obs=obs, weight='std',scaleVar =F)
  return(cost_1)
}

M07b<-function(y0,pars,times){
  out <- ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='muller2003b',nout=4);
  return(out);
}
M07bcost <- function(pars,y0,times,obs){
  out <- ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='muller2003b',nout=4);
  cost_1 <- modCost(model=out, obs=obs, weight='std',scaleVar =F)
  return(cost_1);
}

M07c<-function(y0,pars,times){
  out <- ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='muller2003c',nout=4);
  return(out);
}
M07ccost<-function(y0,pars,times,obs){
  out <- M07c(y0,pars,times);
  cost_1 <- modCost(model=out, obs=obs, weight='std',scaleVar =F);
  return(cost_1);
}

Z13cost <- function (pars,y0,times,obs) {
  out <- ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='zhang2013',nout=4);
  cost_1 <- modCost(model=out, obs=obs, weight='std',scaleVar =F)
  return(cost_1)
}
#model 
Z13<-function(y0,pars,times){
  out <- ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='zhang2013',nout=4);
  return(out);
}
#model
Z13bcost <- function (pars,y0,times,obs) {
  out <- ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='zhang2013b',nout=4);
  cost_1 <- modCost(model=out, obs=obs, weight='std',scaleVar =F)
  return(cost_1)
}
#model 
Z13b<-function(y0,pars,times){
  out <- ode(y=y0,parms=pars,times=times,method='ode45',func='derivs',initfunc='initmod',dllname='zhang2013b',nout=4);
  return(out);
}
...
mdMCMC<-function(ff=M07cost,pp,obs,niter=35000,y0=y0,times,wvar0=0.1,updatecov=100,lower,upper,burninlength=10000)
{
  ...
  pp=unlist(summary(MCMC1)[6,]);
  MCMC <- modMCMC(f=ff, p=pp, niter=niter, y=y0,times=times,obs=obs,jump=Cov0,ntrydr = 3,
...
mdMCMC1<-function(ff=M07cost,pp1,obs,niter1=35000,niter2=50000,y0=y0,times,wvar0=0.1,updatecov=100,lower,upper,burninlength=10000)
{
 ...
 # lower=(summary(MCMC1)[5,])*0.5;
  ...
  MCMC <-modMCMC(f=ff, p=pp, niter=niter2, y=y0,times=times,obs=obs,jump=Cov0,ntrydr = 3,
                  wvar0=wvar0, updatecov=updatecov,lower=lower, upper=upper, burninlength=burninlength);
  if(MCMC$naccepted<=10){
    Fit <- modFit(f=ff,p=pp1,y=y0,times=times,obs=obs,lower=lower,upper=upper,method="Pseudo",control=list(numiter=50000));
    pp <- Fit$par;
...
mdMCMC2<-function(ff=M07cost,pp1,obs,niter1=35000,niter2=50000,y0=y0,times,wvar0=0.1,updatecov=100,lower,upper,burninlength=10000)
{
  ...
  if(MCMC1$naccepted<=10){
    ...
    MCMC1 <- modMCMC(f=ff, p=pp1, niter=niter2, y=y0,times=times,obs=obs,ntrydr = 3,
  ...
  MCMC <-modMCMC(f=ff, p=pp, niter=niter2, y=y0,times=times,obs=obs,jump=Cov0,ntrydr = 3,
                 wvar0=wvar0, updatecov=updatecov,lower=lower, upper=upper, burninlength=burninlength);
  if(MCMC$naccepted<=10){
   MCMC <- modMCMC(f=ff, p=pp, niter=niter2*2, y=y0,times=times,obs=obs,ntrydr = 3,var0=0.1,
                    wvar0=wvar0, updatecov=updatecov,lower=lower, upper=upper,burninlength=1000);
  }
  return(MCMC);
}

mdMCMC2a<-function(ff=M07cost,pp1,obs,niter1=35000,niter2=50000,y0=y0,times,wvar0=0.1,updatecov=100,lower,upper,burninlength=10000)
{
  MCMC1 <- modMCMC(f=ff, p=pp1, niter=niter1, y=y0,times=times,obs=obs,ntrydr = 3,
...
mdMCMC3<-function(ff=M07cost,pp1,obs,niter1=35000,niter2=50000,y0=y0,times,wvar0=0.1,updatecov=100,lower,upper,burninlength=10000)
{
  MCMC1 <- modMCMC(f=ff, p=pp1, niter=niter1, y=y0,times=times,obs=obs,ntrydr = 1,
                   wvar0=wvar0,updatecov=updatecov,lower=lower, upper=upper,burninlength=1000);
  ...
  upper=summary(MCMC1)[7,];
  pp=unlist(summary(MCMC1)[6,]);
  MCMC <-modMCMC(f=ff, p=pp, niter=niter2, y=y0,times=times,obs=obs,jump=Cov0,ntrydr = 1,
                 wvar0=wvar0, updatecov=updatecov,lower=lower, upper=upper, burninlength=burninlength);
  if(MCMC$naccepted<=10){
    Fit <- modFit(f=ff,p=pp1,y=y0,times=times,obs=obs,lower=lower,upper=upper,method="Pseudo");
    pp <- Fit$par;
    
    MCMC <- modMCMC(f=ff, p=pp, niter=niter2, y=y0,times=times,obs=obs,ntrydr = 1,
                    wvar0=wvar0, updatecov=updatecov,lower=lower, upper=upper,burninlength=1000);
  }
  return(MCMC);
}
...
mdMCMC4<-function(ff=M07cost,pp1,obs,niter1=35000,niter2=50000,y0=y0,times,wvar0=0.1,updatecov=100,lower,upper,burninlength=10000)
{
  MF<-modFit(f=ff,p=pp1,obs=obs,y0=y0,times=times,lower=lower,upper=upper);
  sP<-summary(MF);
  Covar<-sP$cov.scaled*2.4^2/2;
  s2prior<-sP$modVariance;
  MCMC1 <- modMCMC(f=ff, p=pp1, jump=Covar, var0=s2prior, wvar0=NULL, niter=niter1, y=y0,times=times,obs=obs,updatecov=updatecov,lower=lower, upper=upper,burninlength=1000);
  pp=unlist(summary(MCMC1)[6,]);
  MCMC <-modMCMC(f=ff, p=pp, niter=niter2, y=y0,times=times,obs=obs,updatecov=updatecov,lower=lower, upper=upper, burninlength=burninlength);
  if(MCMC$naccepted<=10){
    Fit <- modFit(f=ff,p=pp1,y=y0,times=times,obs=obs,lower=lower,upper=upper,method="Pseudo");
    pp <- Fit$par;
    
    MCMC <- modMCMC(f=ff, p=pp, niter=niter2, y=y0,times=times,obs=obs,
                    wvar0=wvar0, updatecov=updatecov,lower=lower, upper=upper,burninlength=1000);
  }
  return(MCMC);
}
...
cor_z13am<-function(obs,parms,y0,times){
  ...
  # obs$NH4_2_N2O = parms['alf_NH4_N2O']*obs$N2O;
  # obs$NO3_2_N2O = parms['alf_NO3_N2O']*obs$N2O;
  # obs$NRec_2_N2O = max(0,(1-parms['alf_NH4_N2O']-parms['alf_NO3_N2O']))*obs$N2O;
  
 # obs$NH4_2_N2O_15 = parms['alf_NH4_N2O']*obs$N2O_15;
 # obs$NO3_2_N2O_15 = parms['alf_NO3_N2O']*obs$N2O_15;
 # obs$NRec_2_N2O_15 = max(0,(1-parms['alf_NH4_N2O']-parms['alf_NO3_N2O']))*obs$N2O_15;
