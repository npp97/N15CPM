...
# 
par_tots<-as.data.frame(matrix(NA,ncol=ncol(par_tot),nrow=nlvl*repnum))
names(par_tots)<-names(par_tot)
par_tots<-par_tots[,2:ncol(par_tots)]
par_tots$treatment<-rep(lvl,each=repnum)
...
for (i in 1:nlvl){
#   
 par_tots[((i-1)*repnum+1):(i*repnum),1]<-lvl[i]
 par_nh4s[((i-1)*repnum+1):(i*repnum),1]<-lvl[i]
 par_no3s[((i-1)*repnum+1):(i*repnum),1]<-lvl[i]
 par_nh4no3s[((i-1)*repnum+1):(i*repnum),1]<-lvl[i]
...
#   
 for (j in 3:ncol(par_tot)){
   bb<-bayesboot(par_tot[ii_tot,j],weighted.mean,R=repnum,R2=repnum,.parallel = T)$V1
   par_tots[((i-1)*repnum+1):(i*repnum),j-1]<-bb
#     
   bb<-bayesboot(par_nh4[ii_nh4,j],weighted.mean,R=repnum,R2=repnum,.parallel = T)$V1
   par_nh4s[((i-1)*repnum+1):(i*repnum),j-1]<-bb
#     
   bb<-bayesboot(par_no3[ii_no3,j],weighted.mean,R=repnum,R2=repnum,.parallel = T)$V1
   par_no3s[((i-1)*repnum+1):(i*repnum),j-1]<-bb
#     
   bb<-bayesboot(par_nh4no3[ii_nh4no3,j],weighted.mean,R=repnum,R2=repnum,.parallel = T)$V1
   par_nh4no3s[((i-1)*repnum+1):(i*repnum),j-1]<-bb
 }
}
# 
stopCluster(cl)
...
rates_nh4<-as.data.frame(matrix(NA,ncol=(ncol(rates.nh4s)-1),nrow=nlvl*repnum))
names(rates_nh4)<-names(rates.nh4s)[-1]
rates_nh4$treatment<-rep(lvl,each=repnum)
...
#
 for (i in 1:nlvl){
#
   rates_nh4[((i-1)*repnum+1):(i*repnum),1]<-lvl[i]
   rates_no3[((i-1)*repnum+1):(i*repnum),1]<-lvl[i]
   rates_nh4no3[((i-1)*repnum+1):(i*repnum),1]<-lvl[i]
   rates_ss_nh4[((i-1)*repnum+1):(i*repnum),1]<-lvl[i]
   rates_ss_no3[((i-1)*repnum+1):(i*repnum),1]<-lvl[i]
   rates_ss_nh4no3[((i-1)*repnum+1):(i*repnum),1]<-lvl[i]
...
#
     bb<-bayesboot(rates.nh4s[ii_nh4,j],weighted.mean,R=repnum,R2=repnum,.parallel = T)$V1
     rates_nh4[((i-1)*repnum+1):(i*repnum),j-1]<-bb
#
     bb<-bayesboot(rates.no3s[ii_no3,j],weighted.mean,R=repnum,R2=repnum,.parallel = T)$V1
     rates_no3[((i-1)*repnum+1):(i*repnum),j-1]<-bb
#
     bb<-bayesboot(rates.nh4no3s[ii_nh4no3,j],weighted.mean,R=repnum,R2=repnum,.parallel = T)$V1
     rates_nh4no3[((i-1)*repnum+1):(i*repnum),j-1]<-bb
#
     bb<-bayesboot(rates.ss.nh4s[ii_ss_nh4,j],weighted.mean,R=repnum,R2=repnum,.parallel = T)$V1
     rates_ss_nh4[((i-1)*repnum+1):(i*repnum),j-1]<-bb
#
     bb<-bayesboot(rates.ss.no3s[ii_ss_no3,j],weighted.mean,R=repnum,R2=repnum,.parallel = T)$V1
     rates_ss_no3[((i-1)*repnum+1):(i*repnum),j-1]<-bb
#
     bb<-bayesboot(rates.ss.nh4no3s[ii_ss_nh4no3,j],weighted.mean,R=repnum,R2=repnum,.parallel = T)$V1
     rates_ss_nh4no3[((i-1)*repnum+1):(i*repnum),j-1]<-bb
