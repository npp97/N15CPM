...
pars<-foreach(i=1:lgrpPal,.packages=c('FME','compiler','doBy','hydromad','DEoptim'),.combine=rbind) %dopar% {
...
# for (i in 1:nlvlid){
    ...
    ii_nh4 <- which((obs_al$ID %in% grpPal$ID[i])&(obs_al$lable %in% Lb[1]))
    jk.nh4 <- which((ini_val$ID %in% grpPal$ID[i])&(ini_val$lable %in% Lb[1]))  
...
#    y0.nh4['N2O']<-obs_nh4[1,"N2O"];
    ...
    ii_no3 <- which((obs_al$ID %in% grpPal$ID[i])&(obs_al$lable %in% Lb[2]))
    jk.no3 <- which((ini_val$ID %in% grpPal$ID[i])&(ini_val$lable %in% Lb[2]))  
 ...
 #   n2o0.no3<-coef(lm(N2O~time,data=obs_no3[1:2,]))[1]
    ...
    ii_nh4no3 <- which((obs_al$ID %in% grpPal$ID[i])&(obs_al$lable %in% Lb[3]))
    jk.nh4no3 <- which((ini_val$ID %in% grpPal$ID[i])&(ini_val$lable %in% Lb[3]))  
...
#--------------Parameters
    ...
    #upper=apply(data.frame(rep(1,length(pars0)),upper),1,min);
     #print("---------------NH4-N15")
    try({
    MCMC.NH4<-try(mdMCMC2a(ff=Z13acostL,pp1=pp,obs=obs_nh4,niter1=niter_1,niter2=niter_2,y0=y0.nh4,times,wvar0=0.1,updatecov=100,lower=lower1,upper=upper1,burninlength=burnin.length));
    #print(MCMC.NH4$bestpar)
    if(class(MCMC.NH4) %in% "modMCMC"){
      
      pp["C_NRec_2_NH4"] = MCMC.NH4$bestpar["C_NRec_2_NH4"];
      pp["K_NH4_2_NRec"] = MCMC.NH4$bestpar["K_NH4_2_NRec"]

      pp["C_NH4_2_NLab"]  = MCMC.NH4$bestpar["C_NH4_2_NLab"];
      pp["K_NLab_2_NH4"] = MCMC.NH4$bestpar["K_NLab_2_NH4"];

      pp["K_NH4ads_2_NH4"]  = MCMC.NH4$bestpar["K_NH4ads_2_NH4"];
      pp["K_NH4_2_NH4ads"] = MCMC.NH4$bestpar["K_NH4_2_NH4ads"]

      pp["K_NH4_2_NO3"]  = MCMC.NH4$bestpar["K_NH4_2_NO3"];
    ...
    try({
    MCMC.NO3<-try(mdMCMC2a(ff=Z13acostL,pp1=pp,obs=obs_no3,niter1=niter_1,niter2=niter_2,y0=y0.no3,times,wvar0=0.1,updatecov=100,lower=lower2,upper=upper2,burninlength=burnin.length));
    ...
    if(class(MCMC.NO3) %in% "modMCMC"){
      ...
      pp["K_NO3_2_NRec"] = MCMC.NO3$bestpar["K_NO3_2_NRec"];
      pp["C_NRec_2_NO3"] = MCMC.NO3$bestpar["C_NRec_2_NO3"];
      pp["C_NO3_2_NH4"] = MCMC.NO3$bestpar["C_NO3_2_NH4"];
...
#
    ...
    #print("---------------15NH415NO3-N15\n")
      ...
      try({
      MCMC.NH4NO3<-mdMCMC2a(ff=Z13acostL,pp1=pp,obs=obs_nh4no3,niter1=niter_1,niter2=niter_2,y0=y0.nh4no3,times,wvar0=0.1,updatecov=100,lower=lower3,upper=upper3,burninlength=burnin.length);
      #print(MCMC.NH4NO3$bestpar)
      
      if(class(MCMC.NH4NO3) %in% "modMCMC"){
        pp["C_NO3_2_NH4"] =  MCMC.NH4NO3$bestpar["C_NO3_2_NH4"];
        pp["K_NH4_2_NO3"]  = MCMC.NH4NO3$bestpar["K_NH4_2_NO3"];
  ...
  #    upper4=pp*ufactor;
     ...
     # # print(ss$par)
      try({
      MCMC.TNN<-mdMCMC2a(ff=Z13acostL,pp1=pp,obs=obs_nh4no3,niter1 = niter_1,niter2=niter_2,y0=y0.nh4no3,times,wvar0=0.1,updatecov=100,lower=lower4,upper=upper4,burninlength=burnin.length);
        #})
  ...
  #    lower4=apply(rbind(MCMC.NH4$bestpar,MCMC.NO3$bestpar,MCMC.NH4NO3$bestpar,MCMC$bestpar,MCMC.TNN$bestpar),2,FUN=min)/lfactorf;
   #   upper4=apply(rbind(MCMC.NH4$bestpar,MCMC.NO3$bestpar,MCMC.NH4NO3$bestpar,MCMC$bestpar,MCMC.TNN$bestpar),2,FUN=median)*ufactorf;      
	...
	upper4=MCMC.TNN$bestpar*ufactorf;
	  ...
	  MCMC<-modMCMC(f=Z13acost_3ll,p=pp,y0_nh4=y0.nh4,y0_no3=y0.no3,y0_n4n3=y0.nh4no3,times=times,obs_nh4=obs_nh4,obs_no3=obs_no3,obs_n4n3=obs_nh4no3,niter=niter_2,wvar0=0.1,updatecov=100,lower=lower4,upper=upper4,burninlength=burnin.length);
	  ...
	  ss.nh4no3 <- (Z13a(y0.nh4no3,MCMC$bestpar, times))
	  ss.nh4<-(Z13a(y0.nh4,MCMC$bestpar,times))
	  ss.no3<-(Z13a(y0.no3,MCMC$bestpar,times))

#	  sss.nh4no3 <- (M07(y0.nh4no3,de.6$par, times))
#	  sss.nh4<-(M07(y0.nh4,de.6$par,times))
#	  sss.no3<-(M07(y0.no3,de.6$par,times))
	
#	  s3.nh4no3 <- (M07(y0.nh4no3,de.nh4no3$par, times))
#	  s3.nh4<-(M07(y0.nh4,de.nh4$par,times))
#	  s3.no3<-(M07(y0.no3,de.no3$par,times))
...
#	  sss.no3<-M07(y0.no3,MCMC.TNN$bestpar,times)
	  ...
	  s4.nh4no3 <- Z13a(y0.nh4no3, MCMC.NH4NO3$bestpar,times)
	  s4.nh4<-Z13a(y0.nh4,MCMC.NH4$bestpar,times)
	  s4.no3<-Z13a(y0.no3,MCMC.NO3$bestpar,times)
