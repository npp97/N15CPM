...
#    n2o0.nh4<-coef(lm(N2O~time,data=obs_nh4[1:2,]))[1]
    ...
    obs_nh40<-c(time=0,NH4=nh40.nh4,NO3=no30.nh4,NH4_15=nh4_15_0.nh4,NO3_15=no3_15_0.nh4);#,N2O=n2o0.nh4);#,NH4_14=nh4_14_0.nh4,NO3_14=no3_14_0.nh4);
...
#    y0.nh4['N2O']<-obs_nh4[1,"N2O"];
 ...
 #   n2o0.no3<-coef(lm(N2O~time,data=obs_no3[1:2,]))[1]
    ...
    obs_no30<-c(time=0,NH4=nh40.no3,NO3=no30.no3,NH4_15=nh4_15_0.no3,NO3_15=no3_15_0.no3);#,N2O=n2o0.no3);#,NH4_14=nh4_14_0.no3,NO3_14=no3_14_0.no3)
    ...
    obs_nh4no3 <- obs_al[ii_nh4no3, 3:7]
...
#    n2o0.nh4no3<-coef(lm(N2O~time,data=obs_nh4no3[1:2,]))[1]
    ...
    obs_nh4no30<-c(time=0,NH4=nh40.nh4no3,NO3=no30.nh4no3,NH4_15=nh4_15_0.nh4no3,NO3_15=no3_15_0.nh4no3);#,N2O=n2o0.nh4no3);#,NH4_14=nh4_14_0.nh4no3,NO3_14=no3_14_0.nh4no3);
...
#--------------Parameters
    ...
    #upper=apply(data.frame(rep(1,length(pars0)),upper),1,min);
     #print("---------------NH4-N15")
    try({
    MCMC.NH4<-try(mdMCMC2a(ff=Z13amcostL,pp1=pp,obs=obs_nh4,niter1=niter_1,niter2=niter_2,y0=y0.nh4,times,wvar0=0.1,updatecov=100,lower=lower1,upper=upper1,burninlength=burnin.length));
    ...
    print(MCMC.NH4$bestpar);
     #de.nh4<-SCEoptim(M07costL,pp,obs=obs_nh4,y0=y0.nh4,times=times,lower=lower1,upper=upper1,control=list(maxit=maxit.sce1)) 
    ...
    try({
    MCMC.NO3<-try(mdMCMC2a(ff=Z13amcostL,pp1=pp,obs=obs_no3,niter1=niter_1,niter2=niter_2,y0=y0.no3,times,wvar0=0.1,updatecov=100,lower=lower2,upper=upper2,burninlength=burnin.length));
    ...
    if(class(MCMC.NO3) %in% "modMCMC"){
      ...
      pp["K_NO3sto_2_NO3"] = MCMC.NO3$bestpar["K_NO3sto_2_NO3"];
      pp["K_NO3_2_NO3sto"] = MCMC.NO3$bestpar["K_NO3_2_NO3sto"];
#
      print("-----------Best Fitted Parameters  of NO3-N15----------------------")
      print(MCMC.NO3$bestpar);
      #print(MCMC.NO3$bestfunp);
      #de.no3<-DEoptim(fn=Z13amcostL,lower=MCMC.NO3$bestpar/100,upper=MCMC.NO3$bestpar*100,obs=obs_no3,y0=y0.no3,times,DEoptim.control(NP=150,itermax=1000,CR=0.9))
      #de.no3<-SCEoptim(M07costL,pp,obs=obs_no3,y0=y0.no3,times=times,lower=lower2,upper=upper2,control=list(maxit=maxit.sce1)) 
      
      #print(de.no3$par)
    }
    })
    
    #print("---------------15NH415NO3-N15\n")
      lower3=pp/lfactor;
      upper3=pp*ufactor;
      
      try({
      MCMC.NH4NO3<-mdMCMC2a(ff=Z13amcostL,pp1=pp,obs=obs_nh4no3,niter1=niter_1,niter2=niter_2,y0=y0.nh4no3,times,wvar0=0.1,updatecov=100,lower=lower3,upper=upper3,burninlength=burnin.length);
      #print(MCMC.NH4NO3$bestpar)
      

      if(class(MCMC.NH4NO3) %in% "modMCMC"){
      
      if(Lb3[3] %in% 'NH4NO3'){
        pp["Vmax_NO3_2_NH4"] = MCMC.NH4NO3$bestpar["Vmax_NO3_2_NH4"];
        pp["Km_NO3_2_NH4"] = MCMC.NH4NO3$bestpar["Km_NO3_2_NH4"];
  
        pp["Vmax_NH4_2_NO3"]  = MCMC.NH4NO3$bestpar["Vmax_NH4_2_NO3"];
        pp["Km_NH4_2_NO3"] = MCMC.NH4NO3$bestpar["Km_NH4_2_NO3"];
      }
       
      
      if(Lb3[3] %in% 'Urea'){
        pp["C_NRec_2_NH4"] = MCMC.NH4NO3$bestpar["C_NRec_2_NH4"];
        pp["Vmax_NH4_2_NRec"]  = MCMC.NH4NO3$bestpar["Vmax_NH4_2_NRec"];
        pp["Km_NH4_2_NRec"] = MCMC.NH4NO3$bestpar["Km_NH4_2_NRec"];
        pp["Vmax_NH4_2_NLab"] = MCMC.NH4NO3$bestpar["Vmax_NH4_2_NLab"];
        pp["Km_NH4_2_NLab"] = MCMC.NH4NO3$bestpar["Km_NH4_2_NLab"];
        pp["Vmax_NLab_2_NH4"] = MCMC.NH4NO3$bestpar["Vmax_NLab_2_NH4"];
        pp["Km_NLab_2_NH4"] = MCMC.NH4NO3$bestpar["Km_NLab_2_NH4"];
        pp["K_NH4ads_2_NH4"]  = MCMC.NH4NO3$bestpar["K_NH4ads_2_NH4"];
        pp["K_NH4_2_NH4ads"] = MCMC.NH4NO3$bestpar["K_NH4_2_NH4ads"];
        pp["Vmax_NH4_2_NO3"]  = MCMC.NH4NO3$bestpar["Vmax_NH4_2_NO3"];
        pp["Km_NH4_2_NO3"] = MCMC.NH4NO3$bestpar["Km_NH4_2_NO3"];
        pp["Km_NO3_2_NRec"] = MCMC.NH4NO3$bestpar["Km_NO3_2_NRec"];
        pp["Vmax_NO3_2_NRec"] = MCMC.NH4NO3$bestpar["Vmax_NO3_2_NRec"];
        pp["C_NRec_2_NO3"] = MCMC.NH4NO3$bestpar["C_NRec_2_NO3"];
        } 
        #print("-----------SUMMARY of Fitted Parameters of NH4NO3------------------")
        # ig <- which(diff(MCMC.NH4NO3$SS) <= 0)
        # print(summary(as.mcmc(MCMC.NH4NO3$pars[ig, ])))
        print("-----------Best Fitted Parameters of NH4NO3----------------------")
        print(MCMC.NH4NO3$bestpar);
        #de.nh4no3<-DEoptim(fn=Z13amcostL,lower=MCMC.NH4NO3$bestpar/100,upper=MCMC.NH4NO3$bestpar*100,obs=obs_nh4no3,y0=y0.nh4no3,times,DEoptim.control(NP=150,itermax=1000,CR=0.9))
        #de.nh4no3<-SCEoptim(M07costL,pp,obs=obs_nh4no3,y0=y0.nh4no3,times=times,lower=lower3,upper=upper3,control=list(maxit=maxit.sce1)) 
        #print(de.nh4no3$par)
         #print(MCMC.NH4NO3$bestfunp);
      }
      })
    ...
    upper4=pp*ufactor;
     try({
    MCMC.NH4<-try(mdMCMC2a(ff=Z13amcostL,pp1=pp,obs=obs_nh4,niter1=niter_1,niter2=niter_2,y0=y0.nh4,times,wvar0=0.1,updatecov=100,lower=lower4,upper=upper4,burninlength=burnin.length));
    #print(MCMC.NH4$bestpar)
    if(class(MCMC.NH4) %in% "modMCMC"){
     ...
     # # print(ss$par)
      try({
      MCMC.TNN<-mdMCMC2a(ff=Z13amcostL,pp1=pp,obs=obs_nh4no3,niter1 = niter_1,niter2=niter_2,y0=y0.nh4no3,times,wvar0=0.1,updatecov=100,lower=lower4,upper=upper4,burninlength=burnin.length);
        #})
      if(class(MCMC.TNN) %in% "modMCMC"){
        print("-----------SUMMARY of Fitted Parameters of Total------------------")
        #ig <- which(diff(MCMC.TNN$SS) <= 0)
        #print(summary(as.mcmc(MCMC.TNN$pars[ig, ])))
        print("-----------Best Fitted Parameters of Total----------------------")
        print(MCMC.TNN$bestpar);
        #de.TNN<-DEoptim(fn=Z13amcostL,lower=MCMC.TNN$bestpar/100,upper=MCMC.TNN$bestpar*100,obs=obs_nh4no3,y0=y0.nh4no3,times,DEoptim.control(NP=150,itermax=1000,CR=0.9))
        #e.TNN<-SCEoptim(M07costL,pp,obs=obs_nh4no3,y0=y0.nh4no3,times=times,lower=lower4,upper=upper4,control=list(maxit=maxit.sce1)) 
  ...
  #    lower4=apply(rbind(MCMC.NH4$bestpar,MCMC.NO3$bestpar,MCMC.NH4NO3$bestpar,MCMC$bestpar,MCMC.TNN$bestpar),2,FUN=min)/lfactorf;
   #   upper4=apply(rbind(MCMC.NH4$bestpar,MCMC.NO3$bestpar,MCMC.NH4NO3$bestpar,MCMC$bestpar,MCMC.TNN$bestpar),2,FUN=median)*ufactorf;      
	...
	lower4=MCMC.TNN$bestpar/lfactorf;
	upper4=MCMC.TNN$bestpar*ufactorf;
	  ...
	  MCMC<-modMCMC(f=Z13amcost_3ll,p=pp,y0_nh4=y0.nh4,y0_no3=y0.no3,y0_n4n3=y0.nh4no3,times=times,obs_nh4=obs_nh4,obs_no3=obs_no3,obs_n4n3=obs_nh4no3,niter=niter_2,wvar0=0.1,updatecov=100,lower=lower4,upper=upper4,burninlength=burnin.length);
      ...
      if(class(MCMC) %in% "modMCMC"){
       ...
       # de.6<-SCEoptim(Z13amcost_3ll,pp,lower=lower4,upper=upper4,obs_n4n3=obs_nh4no3,obs_nh4=obs_nh4,obs_no3=obs_no3,y0_n4n3=y0.nh4no3,y0_nh4=y0.nh4,y0_no3=y0.no3,times,control=list(maxit=maxit.sce1))
	  ...
	  ss.nh4no3 <- (Z13am(y0.nh4no3,MCMC$bestpar, times))
	  ss.nh4<-(Z13am(y0.nh4,MCMC$bestpar,times))
	  ss.no3<-(Z13am(y0.no3,MCMC$bestpar,times))
...
#	  sss.no3<-M07(y0.no3,MCMC.TNN$bestpar,times)
	  ...
	  s4.nh4no3 <- Z13am(y0.nh4no3, MCMC.NH4NO3$bestpar,times)
	  s4.nh4<-Z13am(y0.nh4,MCMC.NH4$bestpar,times)
	  s4.no3<-Z13am(y0.no3,MCMC.NO3$bestpar,times)
	  
	  cor_ss_nh4_obs<-cor(ss.nh4[,c('NH4','NH4_15','NO3','NO3_15')],obs_nh4[,c('NH4','NH4_15','NO3','NO3_15')])
	  cor_ss_no3_obs<-cor(ss.no3[,c('NH4','NH4_15','NO3','NO3_15')],obs_no3[,c('NH4','NH4_15','NO3','NO3_15')])
	  cor_ss_nh4no3_obs<-cor(ss.nh4no3[,c('NH4','NH4_15','NO3','NO3_15')],obs_nh4no3[,c('NH4','NH4_15','NO3','NO3_15')])

	  cor_s4_nh4_obs<-cor(s4.nh4[,c('NH4','NH4_15','NO3','NO3_15')],obs_nh4[,c('NH4','NH4_15','NO3','NO3_15')])
	  cor_s4_no3_obs<-cor(s4.no3[,c('NH4','NH4_15','NO3','NO3_15')],obs_no3[,c('NH4','NH4_15','NO3','NO3_15')])
	  cor_s4_nh4no3_obs<-cor(s4.nh4no3[,c('NH4','NH4_15','NO3','NO3_15')],obs_nh4no3[,c('NH4','NH4_15','NO3','NO3_15')])
...
#	  print(sss.nh4no3)
	  ...
	  print('ss.nh4no3 vs obs_nh4no3')
	  print(c('mcmc.6cv',format(diag(cor_ss_nh4no3_obs),digits=4)))
	  print(c('mcmc.ibm',format(diag(cor_s4_nh4no3_obs),digits=4)))
#	  print(c('sce.6cv',format(diag(cor_sss_nh4no3_obs),digits=4)))
#	  print(c('sce.ibm',format(diag(cor_s3_nh4no3_obs),digits=4)))

	  #iq<-sample.int(n=nrow(MCMC$pars),size=50000)
	  var2sav<-c("ss.nh4","ss.no3","ss.nh4no3","s4.nh4","s4.no3","s4.nh4no3","y0.no3","y0.nh4","y0.nh4no3","MCMC","cor_ss_nh4_obs",
	             ...
	             "cor_ss_no3_obs","cor_ss_nh4no3_obs","cor_s4_nh4_obs","cor_s4_no3_obs","cor_s4_nh4no3_obs","MCMC.TNN",
	             "obs_nh4","obs_no3","obs_nh4no3", "ss.nh4no3","ss.nh4","ss.no3","obs_al", "ini_val","MCMC.NH4","MCMC.NO3","MCMC.NH4NO3")
	  save(
